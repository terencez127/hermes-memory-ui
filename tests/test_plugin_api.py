...
# ---------------------------------------------------------------------------
# Pure utility function tests — no external I/O required
# ---------------------------------------------------------------------------


def test_redact_url_leaves_plain_text_unchanged(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._redact_url("not a url") == "not a url"
    assert module._redact_url("") == ""


def test_redact_url_replaces_userinfo_with_redacted(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    # Build URL with credentials so the test string itself is never a real secret
    url = "https://" + "testuser:testpass" + "@example.com/path"
    result = module._redact_url(url)
...
def test_redact_url_redacts_known_secret_query_params(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._redact_url("https://api.example.com/v1?api_key=abc123&token=secret&foo=bar")
    assert "abc123" not in result
    assert "secret" not in result
    assert "bar" in result  # non-secret param preserved
    assert "[REDACTED]" in result


def test_redact_url_leaves_clean_url_unchanged(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    url = "https://api.example.com/v1/memories?page=1&limit=50"
    assert module._redact_url(url) == url


def test_safe_error_redacts_bearer_token(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    # Use a clearly fabricated test token; _safe_error should mask it
    fake_token = "tok-TESTSECRET9x"
    msg = "request failed: Bearer " + fake_token
    result = module._safe_error(msg)
    assert fake_token not in result
    assert "[REDACTED]" in result


def test_safe_error_redacts_url_embedded_in_message(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    url = "https://" + "admin:pw9x" + "@host.test/v1"
    result = module._safe_error("call to " + url + " failed")
    assert "pw9x" not in result
    assert "[REDACTED]" in result


def test_safe_error_redacts_key_value_credential(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._safe_error("api_key=myverysecretkey is invalid")
    assert "myverysecretkey" not in result
    assert "[REDACTED]" in result


def test_truthy_passes_through_booleans(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._truthy(True) is True
    assert module._truthy(False) is False


def test_truthy_uses_default_for_none_and_empty(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._truthy(None) is False
    assert module._truthy(None, default=True) is True
    assert module._truthy("") is False
    assert module._truthy("", default=True) is True


def test_truthy_false_strings(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    for val in ("false", "False", "FALSE", "0", "no", "off", "No", "OFF"):
        assert module._truthy(val) is False, f"expected False for {val!r}"


def test_truthy_true_strings(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    for val in ("true", "True", "1", "yes", "on", "anything_else"):
        assert module._truthy(val) is True, f"expected True for {val!r}"


def test_truthy_numeric(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._truthy(0) is False
    assert module._truthy(1) is True
    assert module._truthy(42) is True


def test_dig_retrieves_nested_values(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    data = {"a": {"b": {"c": "found"}}}
    assert module._dig(data, "a", "b", "c") == "found"
    assert module._dig(data, "a", "b") == {"c": "found"}


def test_dig_returns_default_for_missing_keys(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    data = {"a": {"b": 1}}
    assert module._dig(data, "x") is None
    assert module._dig(data, "a", "z", default="fallback") == "fallback"
    assert module._dig(data, "a", "b", "c") is None  # traversal into non-dict


def test_memory_limits_returns_defaults_when_config_empty(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    limits = module._memory_limits({})
    assert limits["memory"] == module.DEFAULT_MEMORY_LIMIT
    assert limits["user"] == module.DEFAULT_USER_LIMIT


def test_memory_limits_reads_memory_char_limit(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    limits = module._memory_limits({"memory": {"memory_char_limit": 9999, "user_char_limit": 3333}})
    assert limits["memory"] == 9999
    assert limits["user"] == 3333


def test_memory_limits_falls_back_to_defaults_on_invalid_values(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    limits = module._memory_limits({"memory": {"memory_char_limit": "bad", "user_char_limit": None}})
    assert limits["memory"] == module.DEFAULT_MEMORY_LIMIT
    assert limits["user"] == module.DEFAULT_USER_LIMIT


def test_parse_entries_empty_string_returns_empty_list(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._parse_entries("") == []
    assert module._parse_entries("   ") == []


def test_parse_entries_splits_on_delimiter(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    delimiter = module.ENTRY_DELIMITER
    raw = f"entry one{delimiter}entry two{delimiter}entry three"
    result = module._parse_entries(raw)
    assert result == ["entry one", "entry two", "entry three"]


def test_parse_entries_filters_blank_entries(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    delimiter = module.ENTRY_DELIMITER
    raw = f"entry one{delimiter}   {delimiter}entry three"
    result = module._parse_entries(raw)
    assert result == ["entry one", "entry three"]


def test_safe_like_wraps_text_with_percent(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._safe_like("hello") == "%hello%"


def test_safe_like_strips_percent_and_underscore(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._safe_like("he%llo_world")
    assert "%" not in result[1:-1]
    assert "_" not in result[1:-1]
    assert result.startswith("%")
    assert result.endswith("%")


def test_shape_session_message_basic(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    msg = {"id": 1, "role": "user", "content": "hello"}
    result = module._shape_session_message(msg)
    assert result["id"] == 1
...
def test_shape_session_message_marks_anchor(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    msg = {"id": 42, "role": "assistant", "content": "reply"}
    result = module._shape_session_message(msg, anchor_id=42)
    assert result["anchor"] is True


def test_shape_session_message_no_anchor_for_other_ids(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    msg = {"id": 5, "role": "user", "content": "msg"}
    result = module._shape_session_message(msg, anchor_id=99)
    assert "anchor" not in result


def test_shape_session_message_includes_tool_fields(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    msg = {"id": 1, "role": "tool", "content": "result", "tool_name": "search", "tool_call_id": "call-1"}
    result = module._shape_session_message(msg)
    assert result["tool_name"] == "search"
    assert result["tool_call_id"] == "call-1"


def test_shape_session_message_filters_none_non_content_keys(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    msg = {"id": 1, "role": "user", "content": None, "timestamp": None}
    result = module._shape_session_message(msg)
    assert "content" in result  # content key kept even when None
    assert "timestamp" not in result


def test_unwrap_mem0_results_from_dict_with_results_key(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    response = {"results": [{"memory": "a"}, {"memory": "b"}]}
    assert module._unwrap_mem0_results(response) == [{"memory": "a"}, {"memory": "b"}]


def test_unwrap_mem0_results_from_dict_with_memories_key(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    response = {"memories": [{"memory": "x"}]}
    assert module._unwrap_mem0_results(response) == [{"memory": "x"}]


def test_unwrap_mem0_results_from_list(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    lst = [{"memory": "m1"}, {"memory": "m2"}]
    assert module._unwrap_mem0_results(lst) == lst


def test_unwrap_mem0_results_returns_empty_for_other_types(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._unwrap_mem0_results(None) == []
    assert module._unwrap_mem0_results("string") == []
    assert module._unwrap_mem0_results(42) == []


def test_normalize_mem0_memory_from_string(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._normalize_mem0_memory("raw text memory", 0)
    assert result["memory"] == "raw text memory"
    assert result["id"] == "1"
    assert result["score"] is None


def test_normalize_mem0_memory_from_full_dict(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    item = {"id": "uuid-abc", "memory": "fact", "score": 0.9, "created_at": "2026-01-01", "user_id": "u1"}
    result = module._normalize_mem0_memory(item, 0)
    assert result["id"] == "uuid-abc"
    assert result["memory"] == "fact"
    assert result["score"] == 0.9
    assert result["user_id"] == "u1"


def test_normalize_mem0_memory_uses_text_fallback(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    item = {"id": "x", "text": "alt text"}
    result = module._normalize_mem0_memory(item, 2)
    assert result["memory"] == "alt text"


def test_filter_mem0_memories_applies_limit(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    mems = [{"memory": f"item {i}"} for i in range(10)]
    assert len(module._filter_mem0_memories(mems, None, 3)) == 3


def test_filter_mem0_memories_case_insensitive_search(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    mems = [{"memory": "Python is great"}, {"memory": "Rust is fast"}, {"memory": "python tips"}]
    result = module._filter_mem0_memories(mems, "python", 10)
    assert len(result) == 2
    assert all("python" in m["memory"].lower() for m in result)


def test_normalize_honcho_card_empty_input(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._normalize_honcho_card(None) == []
    assert module._normalize_honcho_card([]) == []
    assert module._normalize_honcho_card("") == []


def test_normalize_honcho_card_list_input(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._normalize_honcho_card(["fact one", "fact two", None])
    assert result == ["fact one", "fact two"]  # None is filtered


def test_normalize_honcho_card_string_input(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._normalize_honcho_card("single fact")
    assert result == ["single fact"]


def test_object_to_dict_passthrough_for_dict(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    d = {"id": "1", "content": "hello"}
    assert module._object_to_dict(d) == d


def test_object_to_dict_uses_model_dump(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)

    class FakeModel:
        def model_dump(self):
            return {"id": "m1", "content": "via model_dump"}

    result = module._object_to_dict(FakeModel())
    assert result == {"id": "m1", "content": "via model_dump"}


def test_object_to_dict_falls_back_to_known_attrs(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    ...
    result = module._object_to_dict(FakeObj())
    assert result["id"] == "obj-1"
    assert result["content"] == "obj content"


def test_json_safe_primitives_pass_through(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._json_safe(None) is None
    assert module._json_safe(True) is True
    assert module._json_safe(42) == 42
    assert module._json_safe(3.14) == 3.14
    assert module._json_safe("hello") == "hello"


def test_json_safe_serializes_nested_structures(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._json_safe({"a": [1, None, "x"]})
    assert result == {"a": [1, None, "x"]}


def test_json_safe_uses_isoformat_for_datetime_like(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    ...
    assert module._json_safe(FakeDate()) == "2026-01-01T00:00:00"


def test_json_safe_falls_back_to_str(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    ...
    assert module._json_safe(Opaque()) == "opaque-value"


def test_json_object_returns_dict_unchanged(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    d = {"key": "value"}
    assert module._json_object(d) is d


def test_json_object_parses_json_string(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._json_object('{"key": "value"}')
    assert result == {"key": "value"}


def test_json_object_returns_empty_for_non_dict_json(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._json_object("[1, 2, 3]") == {}
    assert module._json_object('"just a string"') == {}


def test_json_object_returns_empty_for_invalid_json(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._json_object("not json") == {}
    assert module._json_object("") == {}
    assert module._json_object(None) == {}


def test_mnemosyne_order_clause_uses_first_preferred_available(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._mnemosyne_order_clause(["id", "created_at", "importance"], ["updated_at", "created_at", "id"])
    assert result == 'ORDER BY "created_at" DESC'


def test_mnemosyne_order_clause_returns_empty_when_none_found(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._mnemosyne_order_clause(["id", "content"], ["updated_at", "created_at"]) == ""


def test_mnemosyne_where_returns_empty_for_no_search(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    clause, params = module._mnemosyne_where(None, ["content", "text"], ["content", "text"])
    assert clause == ""
    assert params == []


def test_mnemosyne_where_builds_clause_for_available_columns(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    clause, params = module._mnemosyne_where("hello", ["content", "text"], ["content", "text"])
    assert clause.startswith("WHERE")
    assert '"content" LIKE ?' in clause
    assert '"text" LIKE ?' in clause
    assert len(params) == 2
    assert all("hello" in p for p in params)


def test_mnemosyne_where_skips_unavailable_columns(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    clause, params = module._mnemosyne_where("hello", ["id"], ["content", "text"])
    assert clause == ""
    assert params == []


def test_mnemosyne_fact_text_memoria_facts_key_value(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    row = {"key": "color", "value": "blue"}
    assert module._mnemosyne_fact_text("memoria_facts", row) == "color: blue"


def test_mnemosyne_fact_text_memoria_facts_value_only(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    row = {"value": "some fact", "key": ""}
    assert module._mnemosyne_fact_text("memoria_facts", row) == "some fact"


def test_mnemosyne_fact_text_memoria_instructions(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    row = {"instruction": "always be concise"}
    assert module._mnemosyne_fact_text("memoria_instructions", row) == "always be concise"


def test_mnemosyne_fact_text_triples(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    row = {"subject": "Alice", "predicate": "knows", "object": "Bob"}
    result = module._mnemosyne_fact_text("triples", row)
    assert "Alice" in result
    assert "knows" in result
    assert "Bob" in result


def test_mnemosyne_fact_text_unknown_table_uses_content(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    row = {"content": "generic content"}
    assert module._mnemosyne_fact_text("unknown_table", row) == "generic content"


def test_hindsight_connection_failed_detects_known_messages(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._hindsight_connection_failed(Exception("Connection refused by server"))
    assert module._hindsight_connection_failed(Exception("Cannot connect to host"))
    assert module._hindsight_connection_failed(Exception("Connect call failed"))


def test_hindsight_connection_failed_returns_false_for_other_errors(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert not module._hindsight_connection_failed(Exception("timeout"))
    assert not module._hindsight_connection_failed(Exception("404 not found"))


def test_hindsight_timeout_seconds_returns_configured_value(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._hindsight_timeout_seconds({"timeout": 30}) == 30.0
    assert module._hindsight_timeout_seconds({"timeout": "45.5"}) == 45.5


def test_hindsight_timeout_seconds_defaults_to_120_for_none_or_invalid(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._hindsight_timeout_seconds({}) == 120.0
    assert module._hindsight_timeout_seconds({"timeout": None}) == 120.0
    assert module._hindsight_timeout_seconds({"timeout": "bad"}) == 120.0


def test_unwrap_byterover_data_extracts_nested_data(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    payload = {"data": {"results": [1, 2, 3], "count": 3}}
    assert module._unwrap_byterover_data(payload) == {"results": [1, 2, 3], "count": 3}


def test_unwrap_byterover_data_returns_payload_without_data_key(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    payload = {"results": [1, 2]}
    assert module._unwrap_byterover_data(payload) == payload


def test_unwrap_byterover_data_returns_empty_for_non_dict(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._unwrap_byterover_data(None) == {}
    assert module._unwrap_byterover_data([1, 2]) == {}


def test_load_simple_env_file_parses_key_value_pairs(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    env_file = tmp_path / ".env"
    env_file.write_text("KEY1=value1\nKEY2=value2\n", encoding="utf-8")
    result = module._load_simple_env_file(env_file)
    assert result == {"KEY1": "value1", "KEY2": "value2"}


def test_load_simple_env_file_strips_quotes(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    env_file = tmp_path / ".env"
...
def test_load_simple_env_file_skips_comments_and_blank_lines(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    env_file = tmp_path / ".env"
...
def test_load_simple_env_file_returns_empty_for_missing_file(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    result = module._load_simple_env_file(tmp_path / "nonexistent.env")
    assert result == {}


def test_compact_byterover_excerpt_returns_empty_for_empty_input(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._compact_byterover_excerpt("", "query") == ""
    assert module._compact_byterover_excerpt("  ", None) == ""


def test_compact_byterover_excerpt_truncates_long_text_without_match(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    long_text = "x" * 1000
    result = module._compact_byterover_excerpt(long_text, None)
    assert len(result) < len(long_text)
    assert result.endswith("…")


def test_compact_byterover_excerpt_centers_around_keyword_hit(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    text = "start. " + ("filler " * 50) + "uniqueterm appears here. " + ("more " * 50) + "end."
    result = module._compact_byterover_excerpt(text, "uniqueterm")
    assert "uniqueterm" in result


def test_compact_byterover_answer_returns_empty_for_empty_answer(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    assert module._compact_byterover_answer("query", "") == ""


def test_compact_byterover_answer_extracts_first_line_when_no_facts(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    answer = "Short answer.\nSecond line."
    result = module._compact_byterover_answer("query", answer)
    assert result == "Short answer."


def test_compact_byterover_answer_extracts_matching_fact(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    answer = "## Facts\n- The project owner is Alice.\n- The project codename is Hermes.\n"
    result = module._compact_byterover_answer("who owns the project", answer)
    assert "Alice" in result


def test_compact_byterover_answer_codename_query(monkeypatch, tmp_path):
    module = load_plugin_api(monkeypatch, tmp_path)
    answer = "## Facts\n- The project codename is Phoenix.\n"
    result = module._compact_byterover_answer("what is the codename", answer)
